...
func (d *mysqlDatabase) GenerateStruct(w io.Writer) error {
	...
	fmt.Fprintln(w, `// Code generated by gormc, http://github.com/shuxs/gormc. DO NOT EDIT.`)
	...
	for _, tab := range tables {
		...
		for _, col := range tab.Columns {
			...
			fmt.Fprint(w, "`")
			fmt.Fprintf(w, "// %s", regexp.MustCompile(`\b+`).ReplaceAllString(col.Comment, ""))
			fmt.Fprintln(w)
...
`, tab.Name, structName, tab.Name)

		if hasCreateAt {
			fmt.Fprintf(w, `
// BeforeCreate before create hook, set CreateAt colume to now
...
`, structName)
		...
		if hasUpdateAt {
			fmt.Fprintf(w, `
// BeforeUpdate before update hook, set UpdateAt colume to now
